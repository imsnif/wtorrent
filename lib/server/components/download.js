...
Download.prototype.start = function start (cb) {
  this.client.add(this.magnetUri, {path: this.storagePath}, (torrent) => {
    ...
    this.torrent.on("download", this._updateState.bind(this, torrent))
    this.torrent.on("upload", this._updateState.bind(this, torrent))
...
Download.prototype._updateState = function _updateState (updatedTorrent, updatedStatus) {
  ...
  let status = updatedStatus || progress >= 1 ? "Done" : this.state.status
  this.state = {
      name          : torrent.name,
      id            : torrent.infoHash,
